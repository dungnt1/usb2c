...
class Illusion_AjaxCartWishlistCompare_AjaxcartController extends Mage_Checkout_CartController
{
    /**
                ...
                if (!$this->_getSession()->getNoCartRedirect(true)) {
                    if (!$cart->getQuote()->getHasError()){
                       ...
                       // $this->_getSession()->addSuccess($message);
                        ...
                        $this->loadLayout();
                        $sidebar_block = $this->getLayout()->getBlock('illusion.header.cart');
                        Mage::register('referrer_url', $this->_getRefererUrl());
                        $illusionHeaderCart = $sidebar_block->toHtml();

                        $pimage = Mage::helper('catalog/image')->init($product, 'small_image')->resize(150);

                        $ajaxData['illusion_header_cart'] = $illusionHeaderCart;
                        $ajaxData['sidebar_cart'] = $sidebarCart;
                        $ajaxData['top_link'] = $toplink;
                        $ajaxData['mini_cart'] = $mini_cart;

                        //show or hide confirmbox when add product to cart
...
//            $this->getResponse()->setBody(Mage::helper('core')->jsonEncode($ajaxData));
     ...
     /**
     * override Delete shopping cart item action
     */
    public function deleteAction() {
        if ($this->getRequest()->getParam('callback')) {
            ...
            if($this->getRequest()->getParam('type_del')=='cart' & $this->getRequest()->getParam('count_remove')==1) {
               $cartEmpty = Mage::helper('illusion_ajaxcartwishlistcompare')->getEmptyCartHtml();
            }
            $ajaxData['status'] = 1;
            $ajaxData['top_link'] = $toplink;
            $ajaxData['sidebar_cart'] = $sidebarCart;
    ...
    */
    ...
    /**
    * override updatePost cart item action
    */
