	...
	$.fn.extend({
		...
		formSelect : function(){
	      	return $(this).each(function(){
	      		...
	      		for(var i = 0; i < len; i++){
	      			list.append('<li class="tr_delay fs_medium color_grey" data-action="' + select.children('option').eq(i).val() + '">'+select.children('option').eq(i).text()+'</li>');
	      		}
...
//		},
        stickyMenu : function(){
            ...
            var sticky = new Object(),
                ...
                hHeight = hSticky.height();
            offset = hSticky.prev().offset().top + hSticky.prev().outerHeight(),
                // menuItems = hSticky.find('#mt_megamenu').children('li').children('a'),
                // pTop = menuItems.css('paddingTop'),
                // pBottom = menuItems.css('paddingBottom'),
                body = $('body');

            $(window).resize( function() {
                hSticky.removeAttr('style');
                hHeight = hSticky.height();
                offset = hSticky.prev().offset().top + hSticky.prev().outerHeight();
                // pTop = menuItems.css('paddingTop');
                // pBottom = menuItems.css('paddingBottom');
            ...
            sticky.stickyActivate = function(){
                ...
                // 	needHeight = hSticky.height();
                // });
                header.css('paddingTop',hSticky.height());
                // var getNeedOffset = setInterval(function(){
                // 	if(hSticky.height() == 0) return;
                // 	nextSibling.css('marginTop',hSticky.height());
                // 	if(+nextSibling.css('marginTop').slice(0,-2) == needHeight){
