...
//				if($(pluginsCollection[0]).length) $(pluginsCollection[0]).easytabs({tabActiveClass: "active"});
				...
				if($(pluginsCollection[3]).length){
					$(pluginsCollection[3]).jackBox("init",{
						    ...
						    baseName: "js/illusion/plugins/jackbox",
						    className: ".jackbox",
						    useThumbs: true,
				...
				if($(pluginsCollection[4]).length){
					...
					var follow = $(pluginsCollection[4]).next('.tw_follow'),
						config = {
							username : user,
							modpath: BASE_URL + 'js/illusion/plugins/twitter/',
							loading_text : '<img src="' + BASE_URL + 'skin/frontend/illusion/default/images/loader2.gif" alt=""/>',
...
//				}
				...
				if($(pluginsCollection[8]).length){
					var carousel = $(pluginsCollection[8]),
                        ...
                        var BASE_URL = $('#base').attr('data-base');
						hItems.find('a[data-group="product"]').jackBox("init",{baseName: BASE_URL + "js/illusion/plugins/jackbox"});
			
						$('.open_product').on('click',function(e){
							e.preventDefault();
							var index = $('.flex-active-slide').find('.active').data('index');
			...
			owlCarousel: function(options) {
				...
				$("div.owl-carousel").each(function() {
                    ...
                    var l = config.itemsCustom.length;

                    if(l){

                        var itemsCount = slider.find('.owl-item').length,
                            m = config.itemsCustom[l-1][1];

                        if(m >= 5){
                            slider.find('.fp_item').find('figcaption').addClass('inColumn');
                        }

                        if(itemsCount <= m){
                            $('.'+buttonClass+'prev').add($('.'+buttonClass+'next')).hide();
                        }
					...
					subscribe.on('submit',function(e){
						...
						if(self.find('input[type="email"]').val() == ''){
							...
							$.ajax({
                                ...
                                cache: false,
                                dataType: 'json',
                                url: '/newsletter/subscriber/subscribe/',
                                data: {ajax:1, email:jQuery('#newsletter').val()},
				...
				contactForm : function(){
					...
					cf.on("submit",function(event){
						...
						$.ajax({
                            ...
                            data: {ajax:1, data:self.serialize()},
                            success: function(data) {
                                text = data.msg;
                                var type = data.type;
